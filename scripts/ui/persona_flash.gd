extends CanvasLayer
## PersonaFlash — Persona 5 style dramatic text flash.
## Dark red overlay with bold 3D text, waits for player input to dismiss.

@onready var color_rect: ColorRect = $ColorRect
@onready var label: Label = $ColorRect/Label

var phrases_cap1: Array[String] = [
	"¿Por qué a mí?",
	"Otra vez no...",
	"Déjame en paz",
	"No quiero ir",
...
func _ready():
	...
	visible = false
	add_to_group("persona_flash")
	# 3D depth effect on label: thick outline + shadow
	_setup_label_style()
	_create_hint_label()
...
func _create_hint_label():
	...
	_hint_label.add_theme_color_override("font_color", Color(1.0, 1.0, 1.0, 0.6))
...
func trigger_flash(specific_phrase: String = ""):
	if _is_flashing:
		return
	_is_flashing = true
	...
	var phrase: String
	if specific_phrase != "":
		phrase = specific_phrase
	else:
		phrase = phrases_cap1[randi() % phrases_cap1.size()]

	label.text = phrase
	label.rotation_degrees = randf_range(-5.0, 5.0)

	color_rect.color = Color(0.3, 0.0, 0.0, 0.9)
